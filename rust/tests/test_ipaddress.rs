...
mod tests {
    ...
    pub fn test_summarize() {
        ...
        assert_eq!(
            IPAddress::to_string_vec(&IPAddress::summarize_str(vec!["10.1.0.4/24"]).unwrap()),
            ["10.1.0.0/24"]
        );
        assert_eq!(
            IPAddress::to_string_vec(&IPAddress::summarize_str(vec!["2000:1::4711/32"]).unwrap()),
            ["2000:1::/32"]
        );

        assert_eq!(
            IPAddress::to_string_vec(
                &IPAddress::summarize_str(vec!["10.1.0.4/24", "7.0.0.0/0", "1.2.3.4/4"]).unwrap()
            ),
            ["0.0.0.0/0"]
        );
        assert_eq!(
            IPAddress::to_string_vec(
                &IPAddress::summarize_str(vec![
                    "2000:1::/32",
                    "3000:1::/32",
                    "2000:2::/32",
                    "2000:3::/32",
        ...
        assert_eq!(
            IPAddress::to_string_vec(
                &IPAddress::summarize_str(vec!["10.0.0.0/23", "10.0.2.0/24"]).unwrap()
            ),
            ["10.0.0.0/23", "10.0.2.0/24"]
        );
        assert_eq!(
            IPAddress::to_string_vec(
                &IPAddress::summarize_str(vec!["10.0.0.0/24", "10.0.1.0/24", "10.0.2.0/23"])
                    .unwrap()
            ),
            ["10.0.0.0/22"]
        );
        ...
        assert_eq!(
            IPAddress::to_string_vec(
                &IPAddress::summarize_str(vec!["10.0.0.0/16", "10.0.2.0/24"]).unwrap()
            ),
            ["10.0.0.0/16"]
        );
        ...
        assert_eq!(
            IPAddress::to_string_vec(&IPAddress::summarize(&vec![a1.clone(), a2.clone()])),
            ["10.0.0.0/23"]
        );
