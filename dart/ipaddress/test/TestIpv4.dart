...
import '../IpV4.dart';
    ...
    class IPv4Test {
        ...
        IPv4Test(List<String> invalid_ipv4, List<String> valid_ipv4_range,
    ...
    IPv4Test setup() {
        ...
        ipv4t.valid_ipv4["9.9/17"] =
                                IPv4Prefix(
                                    "9.0.0.9",
                                     17
                                );
        ipv4t.valid_ipv4["100.1.100"] =
                                IPv4Prefix(
                                    "100.1.0.100",
                                     32
                                );
        ipv4t.valid_ipv4["0.0.0.0/0"] =
                                IPv4Prefix(
                                    "0.0.0.0",
                                     0
                                );
        ipv4t.valid_ipv4["10.0.0.0"] =
                                IPv4Prefix(
                                    "10.0.0.0",
                                     32
                                );
        ipv4t.valid_ipv4["10.0.0.1"] =
                                IPv4Prefix(
                                    "10.0.0.1",
                                     32
                                );
        ipv4t.valid_ipv4["10.0.0.1/24"] =
                                IPv4Prefix(
                                    "10.0.0.1",
                                     24
                                );
        ipv4t.valid_ipv4["10.0.0.9/255.255.255.0"] =
                                IPv4Prefix(
                                    "10.0.0.9",
                                     24
                                );
...
void main() {
    ...
    test("test_initialize_format_error", () {
        setup().invalid_ipv4.forEach((i) =>
            expect(true, IPAddress.parse(i).isErr())
        );
    ...
    test("test_method_network_u32", () {
        expect(2886732288,
                   setup().ip.network().host_address.toInt());
    });
   
    test("test_method_broadcast_u32", () {
        expect(2886732543,
                   setup().ip.broadcast().host_address.toInt());
    });
    ...
    test("test_method_split", () {
        ...
        expect(setup().ip.split(1).unwrap(), [setup().ip.network()]);
    ...
    test("test_method_supernet", () {
        ...
        expect("172.16.10.0/23",
                   setup().ip.supernet(23).unwrap().to_string());
        expect("172.16.8.0/22",
    ...
    test("test_classmethod_parse_u32", () {
        setup().decimal_values.forEach((addr,value) {
            ...
            final ip2 = ip.change_prefix_int(int.parse(splitted.first)).unwrap();
    ...
    test("test_classmethod_summarize", () {
        ...
        expect(IPAddress.summarize([setup().ip]),
                   [setup().ip.network()]);
