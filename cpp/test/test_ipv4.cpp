...
int main() {
  describe("Ipv4Test", []() {
      ...
      it("test_method_each", []() {
          ...
          assert.deepEqual(arr, {"10.0.0.0", "10.0.0.1", "10.0.0.2", "10.0.0.3", "10.0.0.4", "10.0.0.5",
              "10.0.0.6", "10.0.0.7"});
          }
        );
      it("test_method_size", []() {
      ...
      it("test_method_dns_rev_domains", []() {
          assert.deepEqual(IPAddress::parse("173.17.5.1/23")->dns_rev_domains(),
              {"4.17.173.in-addr.arpa", "5.17.173.in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("173.17.1.1/15")->dns_rev_domains(),
              {"16.173.in-addr.arpa", "17.173.in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("173.17.1.1/7")->dns_rev_domains(),
          ...
          assert.deepEqual(IPAddress::parse("174.17.1.1/24")->dns_rev_domains(),
              {"1.17.174.in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("175.17.1.1/16")->dns_rev_domains(),
              {"17.175.in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("176.17.1.1/8")->dns_rev_domains(),
              {"176.in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("177.17.1.1/0")->dns_rev_domains(),
              {"in-addr.arpa"});
          assert.deepEqual(IPAddress::parse("178.17.1.1/32")->dns_rev_domains(),
      ...
      it("test_method_plus", []() {
          ...
          assert.deepEqual(IPAddress::to_string_vec(ip1.add(ip2)),
      ...
      it("test_method_split", []() {
          ...
          assert.deepEqual(IPAddress::to_string_vec(setup().ip.split(1).unwrap()),
          ...
          assert.deepEqual(IPAddress::to_string_vec(setup().network.split(2).unwrap()),
              {"172.16.10.0/25", "172.16.10.128/25"});
          assert.deepEqual(IPAddress::to_string_vec(setup().network.split(1).unwrap()),
              {"172.16.10.0/24"});
      }
      );
      it("test_method_subnet", []() {
          ...
          assert.deepEqual(IPAddress::to_string_vec(setup().network.subnet(26).unwrap()),
              ...
              "172.16.10.128/26",
              "172.16.10.192/26"});
          assert.deepEqual(IPAddress::to_string_vec(setup().network.subnet(25).unwrap()),
              {"172.16.10.0/25", "172.16.10.128/25"});
          assert.deepEqual(IPAddress::to_string_vec(setup().network.subnet(24).unwrap()),
              {"172.16.10.0/24"});
          }
        );
      ...
      it("test_classmethod_summarize", []() {
          ...
          assert.deepEqual(IPAddress::to_string_vec(IPAddress::summarize({setup().ip})),
              {setup().ip.network().to_string()});

          // Summarize homogeneous networks
          auto ip1 = IPAddress::parse("172.16.10.1/24").unwrap();
          auto ip2 = IPAddress::parse("172.16.11.2/24").unwrap();
          assert.deepEqual(IPAddress::to_string_vec(IPAddress::summarize({ip1, ip2})),
              {"172.16.10.0/23"});
