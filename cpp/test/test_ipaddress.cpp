...
void assertArrayEqual(std::vector<std::string> a, std::vector<std::string> b) { assert.deepEqual(a, b); }
...
int main() {
  describe("IPAddressTest", []() {
    ...
    it("test_summarize", []() {
      ...
      for (auto range : {Range(1, 10), Range(11, 127), Range(128, 169), Range(170, 172), Range(173, 192), Range(193, 224)}) {
      ...
      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"10.1.0.4/24"}).unwrap()), {"10.1.0.0/24"});
      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"2000:1::4711/32"}).unwrap()), {"2000:1::/32"});

      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"10.1.0.4/24", "7.0.0.0/0", "1.2.3.4/4"}).unwrap()),
                       {"0.0.0.0/0"});
      assertArrayEqual(
          IPAddress::to_string_vec(IPAddress::summarize_str({"2000:1::/32", "3000:1::/32", "2000:2::/32", "2000:3::/32", "2000:4::/32",
                                                             "2000:5::/32", "2000:6::/32", "2000:7::/32", "2000:8::/32"})
                                       .unwrap()),
          {"2000:1::/32", "2000:2::/31", "2000:4::/30", "2000:8::/32", "3000:1::/32"});

      assertArrayEqual(
          IPAddress::to_string_vec(IPAddress::summarize_str({"10.0.1.1/24", "30.0.1.0/16", "10.0.2.0/24", "10.0.3.0/24", "10.0.4.0/24",
                                                             "10.0.5.0/24", "10.0.6.0/24", "10.0.7.0/24", "10.0.8.0/24"})
                                       .unwrap()),
          {"10.0.1.0/24", "10.0.2.0/23", "10.0.4.0/22", "10.0.8.0/24", "30.0.0.0/16"});

      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"10.0.0.0/23", "10.0.2.0/24"}).unwrap()),
                       {"10.0.0.0/23", "10.0.2.0/24"});
      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"10.0.0.0/24", "10.0.1.0/24", "10.0.2.0/23"}).unwrap()),
                       {"10.0.0.0/22"});

      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize_str({"10.0.0.0/16", "10.0.2.0/24"}).unwrap()), {"10.0.0.0/16"});
      ...
      for (size_t _ = 0; _ < cnt; ++_) {
        assert.deepEqual(
            ...
            {"1.0.0.0/8",      "2.0.0.0/7",      "4.0.0.0/6",      "8.0.0.0/7",      "11.0.0.0/8",     "12.0.0.0/6",     "16.0.0.0/4",
             ...
             "128.0.0.0/3",    "160.0.0.0/5",    "168.0.0.0/8",    "169.0.0.0/9",    "169.128.0.0/10", "169.192.0.0/11", "169.224.0.0/12",
             "169.240.0.0/13", "169.248.0.0/14", "169.252.0.0/15", "169.255.0.0/16", "170.0.0.0/7",    "172.0.0.0/12",   "172.32.0.0/11",
             "172.64.0.0/10",  "172.128.0.0/9",  "173.0.0.0/8",    "174.0.0.0/7",    "176.0.0.0/4",    "192.0.0.0/9",    "192.128.0.0/11",
             "192.160.0.0/13", "192.169.0.0/16", "192.170.0.0/15", "192.172.0.0/14", "192.176.0.0/12", "192.192.0.0/10", "193.0.0.0/8",
      ...
      assertArrayEqual(IPAddress::to_string_vec(IPAddress::summarize({a1, a2})), {"10.0.0.0/23"});
