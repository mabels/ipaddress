...
class Ipv4 {
 public:
  static Result<IPAddress> from_number(const Crunchy& addr, size_t prefix_num) {
    ...
    IPAddress ret(IpBits::v4(), addr.clone(), prefix.unwrap(), None<IPAddress>(), Ipv4::ipv4_is_private, Ipv4::ipv4_is_loopback,
                  Ipv4::to_ipv6);
    return Ok(ret);
  }
  ...
  static Result<IPAddress> create(const std::string& str) {
    ...
    IPAddress ret(IpBits::v4(), split_number.unwrap(), ip_prefix.unwrap(), None<IPAddress>(), Ipv4::ipv4_is_private, Ipv4::ipv4_is_loopback,
                  Ipv4::to_ipv6);
    return Ok(ret);
  }

  static bool ipv4_is_private(const IPAddress& my) {
    for (auto ipa : {IPAddress::parse("10.0.0.0/8").unwrap(), IPAddress::parse("169.254.0.0/16").unwrap(),
                     IPAddress::parse("172.16.0.0/12").unwrap(), IPAddress::parse("192.168.0.0/16").unwrap()}) {
      if (ipa.includes(my)) {
        return true;
      }
    }
  ...
  static bool ipv4_is_loopback(const IPAddress& my) { return IPAddress::parse("127.0.0.0/8").unwrap().includes(my); }
  ...
  static bool is_class_b(const IPAddress& my) {
    return my.is_ipv4() && Crunchy::from_string("80000000", 16).unwrap().lte(my.host_address) &&
  ...
  static bool is_class_c(const IPAddress& my) {
    return my.is_ipv4() && Crunchy::from_string("c0000000", 16).unwrap().lte(my.host_address) &&
