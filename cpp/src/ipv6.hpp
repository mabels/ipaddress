...
class Ipv6 {
 public:
  ...
  static Result<IPAddress> from_int(const Crunchy& adr, size_t prefix_num) {
    ...
    IPAddress ret(IpBits::v6(), adr, prefix.unwrap(), None<IPAddress>(), Ipv6::ipv6_is_private, Ipv6::ipv6_is_loopback, Ipv6::to_ipv6);
  ...
  static Result<IPAddress> create(const std::string& str) {
    ...
    if (IPAddress::is_valid_ipv6(r_slash.unwrap().addr)) {
      ...
      IPAddress ret(IpBits::v6(), o_num.unwrap().crunchy, prefix.unwrap(), None<IPAddress>(), Ipv6::ipv6_is_private, Ipv6::ipv6_is_loopback,
                    Ipv6::to_ipv6);
      return Ipv6::enhance_if_mapped(ret);
    } else {
  ...
  static IPAddress to_ipv6(const IPAddress& ia) { return ia.clone(); }
  ...
  static bool ipv6_is_private(const IPAddress& my) { return IPAddress::parse("fd00::/8").unwrap().includes(my); }
};
}  // namespace ipaddress
#endif
