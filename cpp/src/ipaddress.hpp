...
class IPAddress {
 public:
  ...
  IPAddress(const IpBits* ip_bits, const Crunchy& host_address, const Prefix& prefix, const Option<IPAddress>& mapped,
            const Is& vt_is_private, const Is& vt_is_loopback, const ToIpv4& vt_to_ipv6)
      : prefix(prefix) {
  ...
  IPAddress clone() const {
    ...
    return IPAddress(this->ip_bits->clone(), this->host_address.clone(), this->prefix.clone(), mapped, this->vt_is_private,
                     this->vt_is_loopback, this->vt_to_ipv6);
  }

  bool lt(const IPAddress& oth) const { return this->cmp(oth) == -1; }

  bool lte(const IPAddress& oth) const { return this->cmp(oth) <= 0; }

  bool gt(const IPAddress& oth) const { return this->cmp(oth) == 1; }

  bool gte(const IPAddress& oth) const { return this->cmp(oth) >= 0; }
  ...
  bool eq(const IPAddress& other) const {
    ...
    // }
    // console.log("************", this);
    return this->ip_bits->version == other.ip_bits->version && this->prefix.eq(other.prefix) && this->host_address.eq(other.host_address);
  }
  bool ne(IPAddress& other) { return !this->eq(other); }
  ...
  IPAddress from(const Crunchy& addr, const Prefix& prefix) const {
    ...
    return IPAddress(this->ip_bits, addr.clone(), prefix.clone(), mapped, this->vt_is_private, this->vt_is_loopback, this->vt_to_ipv6);
  }
  ...
  //     //-> true
  //
  bool is_ipv4() const { return this->ip_bits->version == IpVersion::V4; }
  ...
  //     //-> false
  //
  bool is_ipv6() const { return this->ip_bits->version == IpVersion::V6; }
  ...
  //     //=> false
  //
  static bool is_valid(const std::string addr) { return IPAddress::is_valid_ipv4(addr) || IPAddress::is_valid_ipv6(addr); }
  ...
  static bool is_valid_ipv4(const std::string& addr) { return IPAddress::split_to_u32(addr).isOk(); }
  ...
  static Result<ResultCrunchyParts> split_to_num(const std::string& addr) {
    ...
    if (pre_post.size() == 2) {
      ...
      ResultCrunchyParts ret(pre.unwrap().crunchy.shl(128 - (pre.unwrap().parts * 16)).add(post.unwrap().crunchy), 128 / 16);
      return Ok(ret);
    }
  ...
  static bool is_valid_ipv6(const std::string& addr) { return IPAddress::split_to_num(addr).isOk(); }
  ...
  std::vector<size_t> parts() const { return this->ip_bits->parts(this->host_address); }
  ...
  std::string dns_reverse() const {
    ...
    for (auto i = ((this->prefix.host_prefix() + (this->ip_bits->dns_bits - 1)) / this->ip_bits->dns_bits); i < this->dns_parts().size();
         ++i) {
      // console.log("dns_r", i);
      ret += dot;
  ...
  std::vector<IPAddress> dns_networks() const {
    ...
    auto next_bit_mask = this->ip_bits->bits - ((((this->prefix.host_prefix()) / this->ip_bits->dns_bits)) * this->ip_bits->dns_bits);
  ...
  //
  static std::vector<IPAddress> summarize(const std::vector<IPAddress>& networks) { return IPAddress::aggregate(networks); }
  ...
  bool ip_same_kind(const IPAddress& oth) const { return this->ip_bits->version == oth.ip_bits->version; }

  //  Returns true if the address is an unspecified address
  //
  //  See IPAddress::IPv6.Unspecified for more information
  //
  bool is_unspecified() const { return this->host_address.eq(Crunchy::zero()); }

  //  Returns true if the address is a loopback address
  //
  //  See IPAddress::IPv6.Loopback for more information
  //
  bool is_loopback() const { return this->vt_is_loopback(*this); }
  ...
  bool is_mapped() const {
    auto ret = this->mapped.isSome() && this->host_address.shr(32).eq(Crunchy::one().shl(16).sub(Crunchy::one()));
  ...
  //     //=> true
  //
  static bool is_valid_netmask(const std::string& addr) { return IPAddress::parse_netmask_to_prefix(addr).isOk(); }
  ...
  std::string to_s() const { return this->ip_bits->as_compressed_string(this->host_address); }
  ...
  std::string to_s_uncompressed() const { return this->ip_bits->as_uncompressed_string(this->host_address); }
  ...
  std::string to_hex() const { return this->host_address.toString(16); }

  IPAddress netmask() const { return this->from(this->prefix.netmask(), this->prefix); }
  ...
  //      // => true
  //
  bool is_network() const { return this->prefix.num != this->ip_bits->bits && this->host_address.eq(this->network().host_address); }
  ...
  //      // => "172.16.10.0"
  //
  IPAddress network() const { return this->from(IPAddress::to_network(this->host_address, this->prefix.host_prefix()), this->prefix); }

  static Crunchy to_network(const Crunchy& adr, size_t host_prefix) { return adr.shr(host_prefix).shl(host_prefix); }
  ...
  std::vector<IPAddress> add(const IPAddress& other) const { return IPAddress::aggregate({*this, other}); }
  ...
  //      // => "192.168.100.254"
  //
  IPAddress last() const { return this->from(this->broadcast().host_address.sub(this->ip_bits->host_ofs), this->prefix); }
  ...
  //      // => 8
  //
  Crunchy size() const { return Crunchy::one().shl(this->prefix.host_prefix()); }
  bool is_same_kind(const IPAddress& oth) const { return this->is_ipv4() == oth.is_ipv4() && this->is_ipv6() == oth.is_ipv6(); }
  ...
  bool includes(const IPAddress& oth) const {
    auto ret = this->is_same_kind(oth) && this->prefix.num <= oth.prefix.num &&
  ...
  //      // => true
  //
  bool is_private() const { return this->vt_is_private(*this); }
  ...
  Result<IPAddress> supernet(size_t new_prefix) const {
    ...
    //     new_ip = new_ip << 1;
    // }
    auto ret = this->from(this->host_address, this->prefix.from(new_prefix).unwrap()).network();
    return Ok(ret);
  }
  ...
  //      // => "ac10:0a01"
  //
  IPAddress to_ipv6() const { return this->vt_to_ipv6(*this); }
