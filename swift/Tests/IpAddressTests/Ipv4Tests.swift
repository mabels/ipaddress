...
class Ipv4Tests: XCTestCase {
  ...
  func test_method_dns_rev_domains() {
    ...
    XCTAssertEqual(
      IPAddress.parse("174.17.1.1/24")!.dns_rev_domains(),
      ["1.17.174.in-addr.arpa"])
    XCTAssertEqual(
      IPAddress.parse("175.17.1.1/16")!.dns_rev_domains(),
      ["17.175.in-addr.arpa"])
    XCTAssertEqual(
      IPAddress.parse("176.17.1.1/8")!.dns_rev_domains(),
      ["176.in-addr.arpa"])
    XCTAssertEqual(
      IPAddress.parse("177.17.1.1/0")!.dns_rev_domains(),
      ["in-addr.arpa"])
    XCTAssertEqual(
      IPAddress.parse("178.17.1.1/32")!.dns_rev_domains(),
  ...
  func test_method_split() {
    ...
    XCTAssertEqual(
      IPAddress.to_string_vec(setup().network.split(1)),
      ["172.16.10.0/24"])
  }
  ...
  func test_method_subnet() {
    ...
    XCTAssertEqual(
      ...
      ["172.16.10.0/25", "172.16.10.128/25"])
    XCTAssertEqual(
      IPAddress.to_string_vec(setup().network.subnet(24)),
      ["172.16.10.0/24"])
  }
