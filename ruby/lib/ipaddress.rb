...
class IPAddress
  ...
  def self.is_valid_ipv4(addr)
    !IPAddress.split_to_u32(addr).nil?
  end
  ...
  def self.split_on_colon(addr)
    ...
    ResultCrunchyParts.new(ip, parts_len)
  end
  ...
  def self.is_valid_ipv6(addr)
    !IPAddress.split_to_num(addr).nil?
  end
  ...
  def self.summarize(*networks)
    IPAddress.aggregate(networks.flatten)
  end
  ...
  def self.summarize_str(*netstr)
    ...
    IPAddress.aggregate(vec)
  end

  def ip_same_kind(oth)
    @ip_bits.version == oth.ip_bits.version
  end
  ...
  def self.is_valid_netmask(addr)
    !IPAddress.parse_netmask_to_prefix(addr).nil?
  end
  ...
  def self.parse_netmask_to_prefix(netmask)
    ...
    my_ip = my
    IPAddress.netmask_to_prefix(my_ip.host_address, my_ip.ip_bits.bits)
  end
  ...
  def to_s
    @ip_bits.as_compressed_string(@host_address)
  end
  ...
  def to_s_uncompressed
    @ip_bits.as_uncompressed_string(@host_address)
  end
  ...
  def to_hex
    @host_address.toString(16)
  end

  def netmask
    from(@prefix.netmask, @prefix)
  end
  ...
  #
  def broadcast
    from(network.host_address.add(size.sub(Crunchy.one)), @prefix)
  ...
  #
  def network
    from(IPAddress.to_network(@host_address, @prefix.host_prefix), @prefix)
  end

  def self.to_network(adr, host_prefix)
    adr.shr(host_prefix).shl(host_prefix)
  end
  ...
  def sub(other)
    ...
    other.host_address.clone.sub(@host_address)
  end

  def add_num(num)
    from(@host_address.add(num), @prefix)
  end

  def add(other)
    IPAddress.aggregate([self, other])
  end
  ...
  #
  def first
    from(network.host_address.add(@ip_bits.host_ofs), @prefix)
  end
  ...
  #
  def last
    from(broadcast.host_address.sub(@ip_bits.host_ofs), @prefix)
  end
  ...
  #
  def size
    Crunchy.one.shl(@prefix.host_prefix)
  end
  ...
  def supernet(new_prefix)
    ...
    #      new_ip = new_ip << 1
    #  }
    from(@host_address, @prefix.from(new_prefix)).network
  end
