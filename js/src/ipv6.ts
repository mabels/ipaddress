...
export const Ipv6 = {
  ...
  enhance_if_mapped(ip: IPAddress): IPAddress {
    ...
    // console.log("------C", ip);
    const ipv6_top_96bit = ip.host_address.shr(32);
    // console.log("------D", ip);
    if (ipv6_top_96bit.eq(Crunchy.from_number(0xffff))) {
      // console.log("------E");
      const num = ip.host_address.mod(Crunchy.one().shl(32));
      // console.log("------F");
      if (num.eq(Crunchy.zero())) {
        return ip;
      }
      // println!("ip:{},{:x}", ip.to_string(), num);
      const ipv4_bits = IpBits.v4();
  ...
  from_int(adr: Crunchy, prefix_num: number): IPAddress {
    ...
    const ret = Ipv6.enhance_if_mapped(
      new IPAddress({
        ip_bits: IpBits.v6(),
  ...
  create(str: string): IPAddress {
    ...
    if (IPAddress.is_valid_ipv6(ip)) {
      // console.log("3>>>>>>>>>", str);
      const o_num = IPAddress.split_to_num(ip);
      if (o_num === null) {
      ...
      // console.log("5>>>>>>>>>", str);
      const prefix = Prefix128.create(netmask);
      if (prefix === null) {
  ...
  ipv6_is_loopback(my: IPAddress): boolean {
