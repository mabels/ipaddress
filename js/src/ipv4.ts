...
import Prefix32 from "./prefix32.js";
...
export const Ipv4 = {
  ...
  create(str: string): IPAddress {
    ...
    const ip_prefix = Prefix32.create(ip_prefix_num);
    if (ip_prefix === null) {
      // enable && console.log("xx3");
      return null;
    }
    const split_number = IPAddress.split_to_u32(ip);
  ...
  parse_classful(ip_si: string): IPAddress {
    ...
    const o_ip = IPAddress.parse(ip_si);
    if (o_ip === null) {
      return o_ip;
    }
    const ip = o_ip;
