...
class IPv4Prefix {
  ip: string;
  prefix: number;
  constructor(obj: Ipv4PrefixParams) {
    this.ip = obj.ip;
...
describe("Ipv4Test", () => {
  ...
  function setup(): IPv4Test {
    const ipv4t = new IPv4Test();
    ipv4t.valid_ipv4.push([
      "9.9/17",
      new IPv4Prefix({
        ip: "9.0.0.9",
        prefix: 17,
    ...
    ipv4t.decimal_values.push(["0.0.0.0/0", Crunchy.parse("0")]);
  ...
  it("test_initialize", () => {
    for (const i of setup().valid_ipv4) {
      // console.log(i[0]);
      const ip = IPAddress.parse(i[0]);
      assert.isOk(ip.is_ipv4() && !ip.is_ipv6());
    }
    assert.equal(32, setup().ip.prefix.ip_bits.bits);
    assert.isNull(IPAddress.parse("1.f.13.1/-3"));
    assert.isOk(IPAddress.parse("10.0.0.0/8"));
  });
  it("test_initialize_format_error", () => {
    for (const i of setup().invalid_ipv4) {
      assert.isNull(IPAddress.parse(i[0]));
    }
    assert.isNull(IPAddress.parse("10.0.0.0/asd"));
  });
  it("test_initialize_without_prefix", () => {
    assert.isOk(IPAddress.parse("10.10.0.0"));
    const ip = IPAddress.parse("10.10.0.0");
  ...
  it("test_octets", () => {
    const ip = IPAddress.parse("10.1.2.3/8");
    assertArrayEqual(ip.parts(), [10, 1, 2, 3]);
  });
  ...
  it("test_netmask", () => {
    for (const i of setup().netmask_values) {
      const addr = i[0];
      const mask = i[1];
      const ip = IPAddress.parse(addr);
      assert.equal(ip.netmask().to_s(), mask);
    }
  });
  it("test_method_to_u32", () => {
    for (const i of setup().decimal_values) {
      const addr = i[0];
      const int = i[1];
      const ip = IPAddress.parse(addr);
      assert.isOk(ip.host_address.eq(int));
    }
  });
  ...
  it("test_one_address_network", () => {
    const network = IPAddress.parse("172.16.10.1/32");
    assert.equal(false, network.is_network());
  });
  it("test_method_broadcast", () => {
    for (const i of setup().broadcast) {
      const addr = i[0];
      const bcast = i[1];
      const ip = IPAddress.parse(addr);
  ...
  it("test_method_broadcast_u32", () => {
    assert.equal("2886732543", setup().ip.broadcast().host_address.toString());
  });
  it("test_method_include", () => {
    let ip = IPAddress.parse("192.168.10.100/24");
    const addr = IPAddress.parse("192.168.10.102/24");
    assert.equal(true, ip.includes(addr));
    assert.equal(false, ip.includes(IPAddress.parse("172.16.0.48")));
    ...
    assert.equal(false, ip.includes(IPAddress.parse("172.16.0.0/16")));
    ...
    assert.equal(false, ip.includes(IPAddress.parse("13.16.0.0/32")));
  });
  it("test_method_include_all", () => {
    const ip = IPAddress.parse("192.168.10.100/24");
  ...
  it("test_method_private", () => {
    assert.equal(true, IPAddress.parse("169.254.99.4/24").is_private());
  ...
  it("test_method_compare", () => {
    ...
    const ip4 = IPAddress.parse("10.1.1.1/8");
  ...
  it("test_method_minus", () => {
    const ip1 = IPAddress.parse("10.1.1.1/8");
    const ip2 = IPAddress.parse("10.1.1.10/8");
  ...
  it("test_method_plus", () => {
    ...
    ip2 = IPAddress.parse("10.0.2.0/24");
    assertArrayEqual(IPAddress.to_string_vec(ip1.add(ip2)), ["10.0.0.0/16"]);
  ...
  it("test_method_netmask_equal", () => {
    ...
    const ip2 = ip.change_netmask("255.255.255.0");
  ...
  it("test_method_split", () => {
    ...
    assertArrayEqual(IPAddress.to_string_vec(setup().network.split(7)), [
      "172.16.10.0/27",
      "172.16.10.32/27",
      "172.16.10.64/27",
      "172.16.10.96/27",
    ...
    assertArrayEqual(IPAddress.to_string_vec(setup().network.split(6)), [
      "172.16.10.0/27",
      "172.16.10.32/27",
      "172.16.10.64/27",
      "172.16.10.96/27",
    ...
    assertArrayEqual(IPAddress.to_string_vec(setup().network.split(5)), [
      "172.16.10.0/27",
      "172.16.10.32/27",
      "172.16.10.64/27",
      "172.16.10.96/27",
    ...
    assertArrayEqual(IPAddress.to_string_vec(setup().network.split(4)), [
      "172.16.10.0/26",
      "172.16.10.64/26",
      "172.16.10.128/26",
      "172.16.10.192/26",
  ...
  it("test_method_subnet", () => {
    ...
    assert.isNull(setup().network.subnet(33));
    assert.isOk(setup().ip.subnet(30));
    assertArrayEqual(IPAddress.to_string_vec(setup().network.subnet(26)), [
      "172.16.10.0/26",
      "172.16.10.64/26",
      "172.16.10.128/26",
      "172.16.10.192/26",
  ...
  it("test_classmethod_parse_u32", () => {
    for (const i of setup().decimal_values) {
      const addr = i[0];
      const int = i[1];
      const ip = Ipv4.from_number(int, 32);
      const splitted = addr.split("/");
      const ip2 = ip.change_prefix(parseInt(splitted[1], 10));
  ...
  it("test_classmethod_parse_classful", () => {
    for (const i of setup().classful) {
      const ip = i[0];
      const prefix = i[1];
      const res = Ipv4.parse_classful(ip);
      assert.equal(prefix, res.prefix.num);
      assert.equal(`${ip}/${prefix}`, res.to_string());
    }
    assert.isNull(Ipv4.parse_classful("192.168.256.257"));
  });
});
