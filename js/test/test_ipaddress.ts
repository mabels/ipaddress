...
describe("IPAddressTest", () => {
  ...
  it("test_summarize", function () {
    // this.timeout(15000);
    const netstr: string[] = [];
    for (const range of [Range(1, 10), Range(11, 127), Range(128, 169), Range(170, 172), Range(173, 192), Range(193, 224)]) {
    ...
    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["10.1.0.4/24"])), ["10.1.0.0/24"]);
    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["2000:1::4711/32"])), ["2000:1::/32"]);

    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["10.1.0.4/24", "7.0.0.0/0", "1.2.3.4/4"])), ["0.0.0.0/0"]);
    assertArrayEqual(
    ...
    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["10.0.0.0/23", "10.0.2.0/24"])), [
      "10.0.0.0/23",
      "10.0.2.0/24",
    ]);
    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["10.0.0.0/24", "10.0.1.0/24", "10.0.2.0/23"])), [
      "10.0.0.0/22",
    ]);

    assertArrayEqual(IPAddress.to_string_vec(IPAddress.summarize_str(["10.0.0.0/16", "10.0.2.0/24"])), ["10.0.0.0/16"]);
