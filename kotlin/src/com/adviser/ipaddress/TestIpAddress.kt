...
class IPAddressTest(valid_ipv4: String, valid_ipv6: String, valid_mapped: String,
                    invalid_ipv4: String, invalid_ipv6: String, invalid_mapped: String)  {
    public val valid_ipv4 = valid_ipv4
    public val valid_ipv6 = valid_ipv6
    public val valid_mapped = valid_mapped
    public val invalid_ipv4 = invalid_ipv4
    public val invalid_ipv6 = invalid_ipv6
    public val invalid_mapped = invalid_mapped
}

class Range(start: Int, stop: Int) {
    public val start = start
    public val stop = stop
}

class TestIpAddress {
    
    public fun setup(): IPAddressTest {
    ...
    @Test
    public fun test_method_ipaddress() {
    ...
    @Test
    public fun test_module_method_valid() {
    ...
    public fun test_summarize() {
        ...
        for (range in ranges) {
            for (i in range.start..range.stop) {
                netstr.plus(String.format("%d.0.0.0/8", i));
            }
        }
        for (i in 0..256) {
            if (i != 254) {
                netstr.plus(String.format("169.%d.0.0/16", i));
            }
        }
        for (i in 0..256) {
            if (i < 16 || 31 < i) {
                netstr.plus(String.format("172.%d.0.0/16", i));
            }
        }
        for (i in 0..256) {
            if (i != 168) {
                netstr.plus(String.format("192.%d.0.0/16", i));
            }
        }
        val ip_addresses = emptyArray<IPAddress>()
        for (net in netstr) {
            ip_addresses.plus(IPAddress.parse(net).unwrap());
        }

        val empty_vec = emptyArray<String>();
        assertEquals(IPAddress.summarize_str(empty_vec).unwrap().size, 0);
        val sone = IPAddress.summarize_str(arrayOf("10.1.0.4/24")).unwrap()
        val one = IPAddress.to_string_vec(sone)
        assertArrayEquals(one, arrayOf("10.1.0.0/24"));
        assertArrayEquals(IPAddress.to_string_vec(IPAddress.summarize_str(arrayOf("2000:1::4711/32"))
                       .unwrap()),
                   arrayOf("2000:1::/32"));

        assertArrayEquals(IPAddress.to_string_vec(IPAddress.summarize_str(arrayOf("10.1.0.4/24",
                                                                           "7.0.0.0/0",
                                                                           "1.2.3.4/4"))
                       .unwrap()), arrayOf("0.0.0.0/0"));
        val tmp = IPAddress.to_string_vec(IPAddress.summarize_str(arrayOf("2000:1::/32",
                                                                           "3000:1::/32",
                                                                           "2000:2::/32",
                                                                           "2000:3::/32",
                                                                           "2000:4::/32",
