...
typealias Each = (ipa: IPAddress) -> Void;

class IPAddress(ip_bits: IpBits, host_address: BigInteger, prefix: Prefix,
                mapped: IPAddress?, is_private: VtBool, is_loopback: VtBool,
                to_ipv6: VtIPAddress) {
    public val ip_bits = ip_bits;
    public val host_address = host_address;
    public val prefix = prefix;
    public val mapped: IPAddress? = mapped;
    public val vt_is_private = is_private;
    public val vt_is_loopback = is_loopback;
    public val vt_to_ipv6 = to_ipv6;
    ...
    public fun from(addr: BigInteger, prefix: Prefix): IPAddress {
    ...
    public fun setMapped(mapped: IPAddress?): IPAddress {
        return setMapped(this.host_address, mapped, this.prefix.clone())
    }

    public fun setMapped(hostAddr: BigInteger, mapped: IPAddress?, prefix: Prefix): IPAddress {
        return IPAddress(
                this.ip_bits,
    ...
    public final override fun equals(oth: Any?): Boolean {
        return this.compare(oth as IPAddress) == 0;
    }

    public fun compare(oth: IPAddress): Int {
    ...
    public fun equal(other: IPAddress): Boolean {
    ...
    public fun lt(ipa: IPAddress): Boolean {
        return this.compare(ipa) < 0
    }

    public fun gt(ipa: IPAddress): Boolean {
        return this.compare(ipa) > 0
    }

    companion object {
        public fun sort(ipas: Array<IPAddress>): Array<IPAddress> {
            var ret = ipas.copyOf();
            ret.sortedArrayWith(Comparator { a, b -> a.compare(b); })
            return ret;
        }
        ...
        ///
        public fun parse(str: String): Result<IPAddress> {
        ...
        public fun split_at_slash(str: String): AddrNetmask {
        ...
        class AddrNetmask(addr: String, netmask: String?) {
            public val addr = addr;
            public val netmask = netmask;
        }
        ...
        fun split_to_u32(addr: String): Result<Long> {
            ...
            for (i in split_addr) {
                ...
                ip = ip || (part.unwrap().longValue() < < shift);
                shift -= 8;
            }
            return Result.Ok(ip);
        }
        ...
        ///     // => false
        ///
        class SplitOnColon(ip: BigInteger, size: Int) {
            public val ip = ip;
        ...
        fun split_on_colon(addr: String): Result<SplitOnColon> {
            ...
            for (i in parts) {
                ...
                ip = ip.add(BigInteger.valueOf(part_num.i).shiftLeft(shift));
                shift -= 16;
            }
        ...
        fun split_to_num(addr: String): Result<BigInteger> {
            ...
            if (pre_post.size === 0 && addr.contains("::")) {
        ...
        fun aggregate(networks: Array<IPAddress>): Array<IPAddress> {
            if (networks.size == 0) {
                return Array<IPAddress>();
            }
            if (networks.size == 1) {
                return arrayOf(networks.get(0).network());
            }
        ...
        public fun summarize_str(netstr: Array<String>): Result<Array<IPAddress>> {
        ...
        public fun parseInt(s: String, radix: Int): Int? {
            try {
        ...
        public fun sum_first_found(arr: Array<IPAddress>): Array<IPAddress> {
            var dup = MutableList<IPAddress>(arr); // ..addAll(arr);
            if (dup.size < 2) {
                return dup;
            }
            for (i in (dup.size - 2)..0 step -1) {
                val a = IPAddress.summarize(arrayOf(dup.get(i), dup.get(i + 1)));
        ...
        public fun to_string_vec(vec: Array<IPAddress>): List<String> {
            return vec.map { i -> i.to_string() };
        }

        public fun to_ipaddress_vec(vec: Array<String>): Result<Array<IPAddress>> {
            var ret = Array<IPAddress>()
            for (ipstr in vec) {
        ...
        public fun is_valid_netmask(addr: String): Boolean {
            return IPAddress.parse_netmask_to_prefix(addr).isOk();
        }

        public fun netmask_to_prefix(nm: BigInteger, bits: Int): Result<Int> {
            var prefix = 0;
        ...
        public fun parse_netmask_to_prefix(my_str: String): Result<Int> {
        ...
        public fun summarize(networks: Array<IPAddress>): Array<IPAddress> {
    ...
    ///     //-> false
    ///
    public fun is_ipv6(): Boolean {
        return this.ip_bits.version == IpVersion.V6;
    }

    public fun parts(): IntArray {
        return this.ip_bits.parts(this.host_address);
    }

    public fun parts_hex_str(): Array<String> {
        val parts = this.parts()
        val ret = Array<String>(parts.size);
    ...
    ///
    public fun dns_rev_domains(): Array<String> {
        val dns_networks = this.dns_networks()
        val ret = Array<String>(dns_networks.size);
        for (i in 0..dns_networks.size) {
    ...
    public fun dns_reverse(): String {
        val ret = StringBuilder()
        var dot = "";
        val dns_parts = this.dns_parts();
        for (i in ((this.prefix.host_prefix() + (this.ip_bits.dns_bits - 1)) / this.ip_bits.dns_bits)..dns_parts.size) {
            ret.append(dot);
    ...
    public fun dns_networks(): Array<IPAddress> {
        ...
        while (step.compareTo(this.broadcast().host_address) <= 0) {
            ret.plus(this.from(step, prefix));
    ...
    public fun ip_same_kind(oth: IPAddress): Boolean {
    ...
    public fun is_unspecified(): Boolean {
    ...
    public fun is_loopback(): Boolean {
    ...
    ///      ///  172.16.100.4/22
    ///
    public fun change_prefix(prefix: Prefix): Result<IPAddress> {
        return Result.Ok(this.from(this.host_address, prefix));
    }

    public fun change_prefix(num: Int): Result<IPAddress> {
    ...
    public fun change_netmask(my_str: String): Result<IPAddress> {
    ...
    public fun to_string_uncompressed(): String {
        val ret = StringBuilder()
    ...
    public fun to_s_uncompressed(): String {
        return this.ip_bits.as_uncompressed_string(this.host_address);
    }


    public fun to_s_mapped(): String {
        if (this.is_mapped()) {
    ...
    public fun to_string_mapped(): String {
        if (this.is_mapped()) {
    ...
    public fun bits(): String {
        ...
        var ret = StringBuilder ()
        for (i in num.length..this.ip_bits.bits) {
            ret.append("0");
        }
        ret.append(num);
        return ret.toString();
    ...
    public fun to_hex(): String {
        return this.host_address.toString(16);
    }

    public fun netmask(): IPAddress {
    ...
    public fun broadcast(): IPAddress {
    ...
    public fun add(other: IPAddress): Array<IPAddress> {
        return IPAddress.aggregate(arrayOf(this, other));
    }

    public fun to_s_vec(vec: Array<IPAddress>): List<String> {
    ...
    public fun each_host(func: Each) {
    ...
    public fun is_same_kind(oth: IPAddress): Boolean {
    ...
    public fun includes(oth: IPAddress): Boolean {
    ...
    public fun includes_all(oths: Array<IPAddress>): Boolean {
    ...
    public fun split(subnets: Int): Result<Array<IPAddress>> {
    ...
    public fun supernet(new_prefix: Int): Result<IPAddress> {
    ...
    public fun subnet(subprefix: Int): Result<Array<IPAddress>> {
    ...
    public fun newprefix(num: Int): Result<Prefix> {
        for (i in num..this.ip_bits.bits) {
            var a = Math.floor(Math.log(i as Double) / Math.log(2.0))
            if (a == Math.log(i) / Math.log(2.0)) {
