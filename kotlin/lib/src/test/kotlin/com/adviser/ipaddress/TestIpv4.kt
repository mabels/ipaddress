...
class TestIpv4 {
    ...
    fun test_classmethod_summarize() {
        ...
        assertArrayEquals(
            IPAddress.summarize(listOf(setup().ip)),
        ...
        var ip2 = IPAddress.parse("172.16.11.2/24").unwrap()
        assertArrayEquals(
            IPAddress.to_string_vec(IPAddress.summarize(listOf(ip1, ip2))),
        ...
        assertArrayEquals(
            IPAddress.to_string_vec(IPAddress.summarize(listOf(ip1, ip2, ip3, ip4))),
            listOf("10.0.0.0/22")
        )
        ...
        assertArrayEquals(
            IPAddress.to_string_vec(IPAddress.summarize(listOf(ip4, ip3, ip2, ip1))),
            listOf("10.0.0.0/22")
        )
